...
"""
    Author: Yifei Ren
    Name: WaveDetection_mul
    Version: 2.0
    Update: Add multiple people detection.
    Date: 09/03/2019
    Description: Judge if the person is waving and give the waving hand.
    Note: Use for multiple people.
...
class wave_detection_mul:
    def __init__(self):
        ...
        self.num = 0  # Number of the people
            ...
            if self.is_detect:
                ...
                if len(self.rects) != 0:
                    for rect in self.rects:
                        ...
                        self.rect_height = rect.bottom() - rect.top()
                        ...
                        self.frame_left = self.frame_copy[int(rect.top() - 0.2 * self.rect_height):
                                                          int(rect.bottom() + 0.05 * self.rect_height),
                                          right_copy[0]: right_copy[1]]
                        # Extract right waving information
                        left_copy = self.assign_broad_left(rect)
                        self.frame_right = self.frame_copy[int(rect.top() - 0.2 * self.rect_height):
                                                           int(rect.bottom() + 0.05 * self.rect_height),
                                           left_copy[0]: left_copy[1]]
                        # Draw the extracted area
                        cv2.rectangle(self.frame_copy, (right_copy[0], int(rect.top() - 0.2 * self.rect_height)),
                                      (right_copy[1], int(rect.bottom() + 0.05 * self.rect_height)), (0, 255, 0), 2, 8)
                        cv2.rectangle(self.frame_copy, (left_copy[0], int(rect.top() - 0.2 * self.rect_height)),
                                      (left_copy[1], int(rect.bottom() + 0.05 * self.rect_height)), (255, 0, 0), 2, 8)
                        # Judge if waving
                        if not self.is_wave:
                        ...
                        self.is_wave = False
                        if self.last_flag == 1:
                            cv2.putText(self.frame_copy, 'Number ' + str(self.num) + ': Waving Left Hand',
                                        (0, 40 * self.num), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
                        elif self.last_flag == 0:
                            cv2.putText(self.frame_copy, 'Number ' + str(self.num) + ': Waving Right Hand',
                                        (0, 40 * self.num), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                        elif self.last_flag == 2:
                            cv2.putText(self.frame_copy, 'Number ' + str(self.num) + ': Not Waving',
                                        (0, 40 * self.num), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    ...
    def judge(self, img, flag):
        ...
        # Show the left or right judgement division
        if flag:
